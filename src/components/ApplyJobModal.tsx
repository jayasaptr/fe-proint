...
import { checkApplication, getCaptchaConfig, submitApplication } from "../lib/api/applications";
...
export const ApplyJobModal = ({
  ...
  const [maritalStatuses, setMaritalStatuses] = useState<{ MaritalStId: number, MaritalSt: string }[]>([]);
  ...
  const [identities, setIdentities] = useState([{ card_type_id: "", number: "" }]);
  const [cardTypes, setCardTypes] = useState<{ CardTypeId: number, CardType: string }[]>([]);
  const [isLoadingCardTypes, setIsLoadingCardTypes] = useState(false);
  const [educations, setEducations] = useState([{ edu_level_id: "", major: "", edu_institution_id: "", gpa: "", is_last_education: true }]);
  const [experiences, setExperiences] = useState([{ company_name: "", position: "", job_period_year: "", salary: "" }]);
  const [documents, setDocuments] = useState<{file: File | null, description: string}[]>([{ file: null, description: "" }]);

  const [regencies, setRegencies] = useState<{ CityId: number, CityCode: string, CityName: string }[]>([]);
  ...
  const [birthCities, setBirthCities] = useState<{ CityId: number, CityCode: string, CityName: string }[]>([]);
  ...
  const [eduLevels, setEduLevels] = useState<{ EduLvlId: number, EduLvlCode: string, EduLvlName: string }[]>([]);
  ...
  const [eduInstitutions, setEduInstitutions] = useState<{ EduInsId: number, EduInsCode: string, EduInsName: string }[]>([]);
  ...
  const [openInstitutionIndex, setOpenInstitutionIndex] = useState<number | null>(null);
  const [isLoadingInstitutions, setIsLoadingInstitutions] = useState(false);

  const [provinces, setProvinces] = useState<{ StateId: number, StateCode: string, StateName: string }[]>([]);
  ...
  const lastMaritalElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastProvinceElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastCityElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastEduElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastInstitutionElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastBirthCityElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  const lastRaceElementRef = React.useCallback((node: HTMLDivElement | null) => {
  ...
  React.useEffect(() => {
    ...
    const fetchMaritalStatuses = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/maritalstatuses?search=${searchMarital}&page=${pageMarital}`, {
        ...
        if (response.ok) {
           ...
           setMaritalStatuses(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.MaritalStId));
  ...
  React.useEffect(() => {
    const fetchCardTypes = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/cardtypes`);
        if (response.ok) {
           const data = await response.json();
  ...
  React.useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/questions/grouped?topic_id=2,3`);
        if (response.ok) {
          const data = await response.json();
  ...
  React.useEffect(() => {
    ...
    const fetchRaces = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/races?search=${searchRace}&page=${pageRace}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
           ...
           setRaces(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.RaceId));
             return [...prev, ...uniqueNewData];
           });
           setHasMoreRace((data.pagination?.has_next || false) && newData.length > 0);
        }
      } catch (error: any) {
        if (error.name !== 'AbortError') {
  ...
  React.useEffect(() => {
    ...
    const fetchBirthCities = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/cities?search=${searchBirthCity}&page=${pageBirthCity}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
           ...
           setBirthCities(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.CityId));
             return [...prev, ...uniqueNewData];
           });
           setHasMoreBirthCity((data.pagination?.has_next || false) && newData.length > 0);
        }
      } catch (error: any) {
        if (error.name !== 'AbortError') {
  ...
  React.useEffect(() => {
    ...
    const fetchProvinces = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/states?search=${searchProvince}&sort_by=StateName&page=${pageProvince}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
           ...
           setProvinces(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.StateId));
             return [...prev, ...uniqueNewData];
           });
  ...
  React.useEffect(() => {
    ...
    const fetchCities = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/cities?search=${searchCity}&state_id=${selectedProvinceId}&page=${pageCity}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
  ...
  React.useEffect(() => {
    ...
    const fetchEduLevels = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/edulevels?search=${searchEdu}&page=${pageEdu}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
           ...
           setEduLevels(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.EduLvlId));
             return [...prev, ...uniqueNewData];
           });
  ...
  React.useEffect(() => {
    ...
    const fetchInstitutions = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/eduinstitutions?search=${searchInstitution}&page=${pageInstitution}`, {
          signal: controller.signal
        });
        if (response.ok) {
           const data = await response.json();
           ...
           setEduInstitutions(prev => {
             ...
             const uniqueNewData = newData.filter((i: any) => !existingIds.has(i.EduInsId));
             return [...prev, ...uniqueNewData];
           });
           setHasMoreInstitution((data.pagination?.has_next || false) && newData.length > 0);
        }
      } catch (error: any) {
        if (error.name !== 'AbortError') {
  ...
  const handleNumericQuestionChange = (questionId: number, rawInput: string) => {
  ...
  // Add/Remove Helpers
  const addIdentity = () => setIdentities([...identities, { card_type_id: "", number: "" }]);
  const removeIdentity = (index: number) => setIdentities(identities.filter((_, i) => i !== index));

  const addEducation = () => {
    const newEducations = educations.map(e => ({ ...e, is_last_education: false }));
    setEducations([...newEducations, { edu_level_id: "", major: "", edu_institution_id: "", gpa: "", is_last_education: true }]);
  };
  const removeEducation = (index: number) => {
    const newEducations = educations.filter((_, i) => i !== index);
    if (newEducations.length > 0 && !newEducations.some(e => e.is_last_education)) {
        newEducations[newEducations.length - 1].is_last_education = true;
    }
    setEducations(newEducations);
  };

  const addExperience = () => setExperiences([...experiences, { company_name: "", position: "", job_period_year: "", salary: "" }]);
  const removeExperience = (index: number) => setExperiences(experiences.filter((_, i) => i !== index));

  const addDocument = () => setDocuments([...documents, { file: null, description: "" }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const checkResult = await checkApplication(formData.email, vacancy.VacantPosId);
        ...
        console.warn('Check application failed, proceeding with submit:', checkError);
      }
      ...
      const answers = Object.entries(questionAnswers)
        ...
        .map(([questionId, answer]) => {
          ...
          const group = questionGroups.find(g => g.questions.some(q => q.QuestionId === qId));
          return {
      ...
      data.append("identities", JSON.stringify(identities.filter(i => i.card_type_id && i.number)));
      data.append("educations", JSON.stringify(educations.filter(e => e.edu_level_id && e.edu_institution_id).map(e => ({
          ...e,
          gpa: e.gpa ? parseFloat(e.gpa) : null
      }))));
      data.append("experiences", JSON.stringify(experiences.filter(e => e.company_name && e.position).map(e => ({
          ...e,
      ...
      if (res.success) {
        toast.success(`Lamaran berhasil dikirim! Kode: ${res.data?.candidate_code || ''}`);
        ...
        if (res.errors) {
          ...
          toast.error(res.message || "Gagal mengirim lamaran. Silakan coba lagi.");
        }
        turnstileInstance?.reset();
        setCaptchaToken(null);
      }
      ...
      if (responseData?.errors) {
        ...
        toast.error(responseData?.message || "Terjadi kesalahan sistem saat menghubungi server.");
      }
      turnstileInstance?.reset();
      setCaptchaToken(null);
    } finally {
        ...
        <div className="fixed inset-0 z-[110] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center animate-in fade-in">
          <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center gap-6 max-w-[280px] animate-in zoom-in-95">
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-slate-800 tracking-wide">MENGIRIM LAMARAN</p>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
        <div className="bg-primary p-6 relative shrink-0">
          ...
          <h2 className="text-xl font-bold text-primary-foreground mt-2">Lamar: {vacancy.VacantPositionName}</h2>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <form id="apply-form" onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
                <h3 className="text-lg font-bold text-slate-800 border-b pb-2">1. Informasi Personal</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <Popover open={openBirthCity} onOpenChange={setOpenBirthCity}>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    {formData.birth_city_id
                                        ? birthCities.find((c) => c.CityId.toString() === formData.birth_city_id)?.CityName || formData.birth_city_id
                                        : <span className="text-muted-foreground cursor-pointer">Pilih Tempat Lahir</span>}
                                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                <Command shouldFilter={false}>
                                    ...
                                    />
                                    <CommandList>
                                        {isLoadingBirthCities && birthCities.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                        {!isLoadingBirthCities && birthCities.length === 0 && <CommandEmpty>Kota tidak ditemukan.</CommandEmpty>}
                                        <CommandGroup>
                                        {birthCities.map((city, i) => (
                                            <CommandItem
                                                key={city.CityId}
                                                ref={i === birthCities.length - 1 ? lastBirthCityElementRef : undefined}
                                                value={city.CityName}
                                                onSelect={() => {
                                                    handleSelectChange('birth_city_id', city.CityId.toString());
                                            ...
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        formData.birth_city_id === city.CityId.toString() ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {city.CityName}
                                            </CommandItem>
                                        ))}
                                        {isLoadingBirthCities && birthCities.length > 0 && (
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="date_of_birth" name="date_of_birth" type="date" required value={formData.date_of_birth} onChange={handleInputChange} />
                    </div>
                </div>
                            ...
                            <PopoverTrigger asChild>
                                ...
                                >
                                    {formData.marital_status_id
                                        ? maritalStatuses.find((status) => status.MaritalStId.toString() === formData.marital_status_id)?.MaritalSt || formData.marital_status_id
                                        : <span className="text-muted-foreground cursor-pointer">Pilih Status</span>}
                                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                <Command shouldFilter={false}>
                                    ...
                                    />
                                    <CommandList>
                                        {isLoadingMarital && maritalStatuses.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                        {!isLoadingMarital && maritalStatuses.length === 0 && <CommandEmpty>Status tidak ditemukan.</CommandEmpty>}
                                        <CommandGroup>
                                        {maritalStatuses.map((status, i) => (
                                            <CommandItem
                                                key={status.MaritalStId}
                                                ref={i === maritalStatuses.length - 1 ? lastMaritalElementRef : undefined}
                                                value={status.MaritalSt}
                                                onSelect={() => {
                                                    handleSelectChange('marital_status_id', status.MaritalStId.toString());
                                            ...
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        formData.marital_status_id === status.MaritalStId.toString() ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {status.MaritalSt}
                                            </CommandItem>
                                        ))}
                                        {isLoadingMarital && maritalStatuses.length > 0 && (
                                            <div className="py-2 flex items-center justify-center text-sm text-slate-500">
                                                <Loader2 className="h-4 w-4 animate-spin mr-2" />
                                                Memuat...
                                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <Popover open={openRace} onOpenChange={setOpenRace}>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    {formData.race_id
                                        ? races.find((r) => r.RaceId.toString() === formData.race_id)?.Race || formData.race_id
                                        : <span className="text-muted-foreground cursor-pointer">Pilih Suku/Etnis</span>}
                                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                <Command shouldFilter={false}>
                                    ...
                                    />
                                    <CommandList>
                                        {isLoadingRaces && races.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                        {!isLoadingRaces && races.length === 0 && <CommandEmpty>Suku/Etnis tidak ditemukan.</CommandEmpty>}
                                        <CommandGroup>
                                        {races.map((race, i) => (
                                            <CommandItem
                                                key={race.RaceId}
                                                ref={i === races.length - 1 ? lastRaceElementRef : undefined}
                                                value={race.Race}
                                                onSelect={() => {
                                                    handleSelectChange('race_id', race.RaceId.toString());
                                                    setOpenRace(false);
                                                }}
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        formData.race_id === race.RaceId.toString() ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {race.Race}
                                            </CommandItem>
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="photo" name="photo" type="file" accept="image/jpeg,image/png,image/jpg" onChange={handlePhotoChange} />
                    </div>
                </div>
            </div>
            ...
            {/* 2. Contact Address */}
            <div className="space-y-4">
                <h3 className="text-lg font-bold text-slate-800 border-b pb-2">2. Alamat & Kontak</h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <Label htmlFor="mobile_phone">No. HP/WhatsApp*</Label>
                        <Input id="mobile_phone" name="mobile_phone" required value={formData.mobile_phone} onChange={handleInputChange} placeholder="081234567890" />
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="zip_code">Kode Pos*</Label>
                        <Input id="zip_code" name="zip_code" required value={formData.zip_code} onChange={handleInputChange} placeholder="10220" />
                    </div>
                </div>

                <div className="space-y-2">
                    <Label htmlFor="id_card_address">Alamat Sesuai KTP*</Label>
                    <Textarea id="id_card_address" name="id_card_address" required value={formData.id_card_address} onChange={handleInputChange} placeholder="Jl. Sudirman No 1..." />
                </div>
                        ...
                        <Popover open={openProvince} onOpenChange={setOpenProvince}>
                            <PopoverTrigger asChild>
                                <Button
                                    ...
                                    className="w-full justify-between font-normal px-3 bg-transparent"
                                    id="province"
                                    disabled={isLoadingProvinces && provinces.length === 0 && !searchProvince}
                                >
                                    {formData.province_id
                                        ? provinces.find((p) => p.StateId.toString() === formData.province_id)?.StateName || formData.province_id
                                        : <span className="text-muted-foreground cursor-pointer">Pilih Provinsi</span>}
                                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                <Command shouldFilter={false}>
                                    ...
                                    />
                                    <CommandList>
                                        {isLoadingProvinces && provinces.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                        {!isLoadingProvinces && provinces.length === 0 && <CommandEmpty>Provinsi tidak ditemukan.</CommandEmpty>}
                                        <CommandGroup>
                                        {provinces.map((prov, i) => (
                                            <CommandItem
                                                key={prov.StateId}
                                                ref={i === provinces.length - 1 ? lastProvinceElementRef : undefined}
                                                value={prov.StateName}
                                                onSelect={() => {
                                                    handleProvinceChange(prov.StateId, prov.StateCode);
                                            ...
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        formData.province_id === prov.StateId.toString() ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {prov.StateName}
                                            </CommandItem>
                        ...
                        <Popover open={openCity} onOpenChange={setOpenCity}>
                            <PopoverTrigger asChild>
                                <Button
                                    ...
                                    className="w-full justify-between font-normal px-3 bg-transparent"
                                    id="city"
                                    disabled={!selectedProvinceId || (isLoadingRegencies && regencies.length === 0 && !searchCity)}
                                >
                                    {formData.city_id
                                        ? regencies.find((c) => c.CityId.toString() === formData.city_id)?.CityName || formData.city_id
                                        : <span className="text-muted-foreground cursor-pointer">Pilih Kota/Kabupaten</span>}
                                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                <Command shouldFilter={false}>
                                    ...
                                    />
                                    <CommandList>
                                        {isLoadingRegencies && regencies.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                        {!isLoadingRegencies && regencies.length === 0 && <CommandEmpty>Kota/Kabupaten tidak ditemukan.</CommandEmpty>}
                                        <CommandGroup>
                                        {regencies.map((city, i) => (
                                            <CommandItem
                                                key={city.CityId}
                                                ref={i === regencies.length - 1 ? lastCityElementRef : undefined}
                                                value={city.CityName}
                                                onSelect={() => {
                                            ...
                                            >
                                                <Check
                                                    className={cn(
                                                        "mr-2 h-4 w-4",
                                                        formData.city_id === city.CityId.toString() ? "opacity-100" : "opacity-0"
                                                    )}
                                                />
                                                {city.CityName}
                                            </CommandItem>
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center border-b pb-2">
                    <h3 className="text-lg font-bold text-slate-800">3. Identitas Lengkap</h3>
                    <Button type="button" variant="outline" size="sm" onClick={addIdentity} className="text-xs">
                        <Plus className="w-4 h-4 mr-1" /> Tambah Identitas
                    </Button>
                </div>
                {identities.map((identity, index) => (
                    <div key={index} className="flex flex-col md:flex-row gap-4 items-start bg-slate-50 p-4 rounded-xl border border-slate-100 relative pr-12">
                        <div className="flex-1 space-y-2 w-full">
                            ...
                            >
                                <SelectTrigger>
                                    <SelectValue placeholder={isLoadingCardTypes ? "Memuat..." : "Pilih Jenis"} />
                                </SelectTrigger>
                                <SelectContent className="z-[150]">
                                    {cardTypes.map(type => (
                                        <SelectItem key={type.CardTypeId} value={type.CardTypeId.toString()}>{type.CardType}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
                        ...
                        <Button type="button" variant="ghost" size="icon" className="text-red-500 hover:text-red-700 hover:bg-red-50 absolute top-4 right-2" onClick={() => removeIdentity(index)}>
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center border-b pb-2">
                    <h3 className="text-lg font-bold text-slate-800">4. Riwayat Pendidikan Formal</h3>
                    <Button type="button" variant="outline" size="sm" onClick={addEducation} className="text-xs">
                        <Plus className="w-4 h-4 mr-1" /> Tambah Pendidikan
                    </Button>
                </div>
                {educations.map((edu, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 relative pr-12">
                        <div className="space-y-2">
                            <Label>Tingkat*</Label>
                            <Popover open={openEduIndex === index} onOpenChange={(open) => setOpenEduIndex(open ? index : null)}>
                                <PopoverTrigger asChild>
                                    <Button
                                        variant="outline"
                                        role="combobox"
                                    ...
                                    >
                                        {edu.edu_level_id
                                            ? eduLevels.find((level) => level.EduLvlId.toString() === edu.edu_level_id)?.EduLvlName || edu.edu_level_id
                                            : <span className="text-muted-foreground cursor-pointer">Pilih Tingkat</span>}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                    <Command shouldFilter={false}>
                                        ...
                                        />
                                        <CommandList>
                                            {isLoadingEduLevels && eduLevels.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                            {!isLoadingEduLevels && eduLevels.length === 0 && <CommandEmpty>Tingkat tidak ditemukan.</CommandEmpty>}
                                            <CommandGroup>
                                            {eduLevels.map((lvl, idx) => (
                                                <CommandItem
                                                    key={lvl.EduLvlId}
                                                    ref={idx === eduLevels.length - 1 ? lastEduElementRef : undefined}
                                                    value={lvl.EduLvlName}
                                                    onSelect={() => {
                                                        const newArr = [...educations];
                                                        newArr[index].edu_level_id = lvl.EduLvlId.toString();
                                                ...
                                                >
                                                    <Check
                                                        className={cn(
                                                            "mr-2 h-4 w-4",
                                                            edu.edu_level_id === lvl.EduLvlId.toString() ? "opacity-100" : "opacity-0"
                                                        )}
                                                    />
                                                    {lvl.EduLvlName}
                                                </CommandItem>
                        ...
                        <div className="space-y-2">
                            ...
                            <Popover open={openInstitutionIndex === index} onOpenChange={(open) => {
                                setOpenInstitutionIndex(open ? index : null);
                                if (!open && searchInstitution && !eduInstitutions.some(i => i.EduInsName.toLowerCase() === searchInstitution.toLowerCase())) {
                                    // Allow manual input if closing the popover and there's text typed that isn't exactly in the list
                                    const newArr = [...educations];
                                    newArr[index].edu_institution_id = searchInstitution;
                                    setEducations(newArr);
                                }
                            }}>
                                <PopoverTrigger asChild>
                                    <Button
                                        ...
                                        className="w-full justify-between font-normal px-3 bg-transparent"
                                        onClick={() => setSearchInstitution(eduInstitutions.find(i => i.EduInsId.toString() === edu.edu_institution_id)?.EduInsName || edu.edu_institution_id)}
                                    >
                                        {edu.edu_institution_id
                                            ? eduInstitutions.find((i) => i.EduInsId.toString() === edu.edu_institution_id)?.EduInsName || edu.edu_institution_id
                                            : <span className="text-muted-foreground cursor-pointer">Cari Institusi</span>}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-[--radix-popover-trigger-width] p-0 z-[160]" align="start">
                                    <Command shouldFilter={false}>
                                        ...
                                        />
                                        <CommandList>
                                            {isLoadingInstitutions && eduInstitutions.length === 0 && <CommandEmpty>Memuat...</CommandEmpty>}
                                            {!isLoadingInstitutions && eduInstitutions.length === 0 && <CommandEmpty>Institusi tidak ditemukan. Ketik untuk mencari.</CommandEmpty>}
                                            <CommandGroup>
                                            {eduInstitutions.map((inst, idx) => (
                                                <CommandItem
                                                    key={inst.EduInsId}
                                                    ref={idx === eduInstitutions.length - 1 ? lastInstitutionElementRef : undefined}
                                                    value={inst.EduInsName}
                                                    onSelect={() => {
                                                        const newArr = [...educations];
                                                        newArr[index].edu_institution_id = inst.EduInsId.toString();
                                                        setEducations(newArr);
                                                ...
                                                >
                                                    <Check
                                                        className={cn(
                                                            "mr-2 h-4 w-4",
                                                            edu.edu_institution_id === inst.EduInsId.toString() ? "opacity-100" : "opacity-0"
                                                        )}
                                                    />
                                                    {inst.EduInsName}
                                                </CommandItem>
                                            ))}
                                            {isLoadingInstitutions && eduInstitutions.length > 0 && (
                                                <div className="py-2 flex items-center justify-center text-sm text-slate-500">
                                                    <Loader2 className="h-4 w-4 animate-spin mr-2" />
                                                    Memuat...
                                                </div>
                        ...
                        <div className="col-span-1 md:col-span-2 flex items-center gap-2 mt-2">
                            ...
                            />
                            <Label htmlFor={`last_edu_${index}`} className="font-normal cursor-pointer">Jadikan pendidikan terakhir</Label>
                        </div>
                        <Button
                            type="button"
                            variant="ghost"
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center border-b pb-2">
                    <h3 className="text-lg font-bold text-slate-800">5. Pengalaman Kerja</h3>
                    <Button type="button" variant="outline" size="sm" onClick={addExperience} className="text-xs">
                    ...
                    <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 relative pr-12">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    type="text"
                                    value={exp.salary ? Number(exp.salary).toLocaleString('id-ID') : ''}
                                    onChange={(e) => {
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center border-b pb-2">
                    <h3 className="text-lg font-bold text-slate-800">6. Dokumen Tambahan</h3>
                    <Button type="button" variant="outline" size="sm" onClick={addDocument} className="text-xs">
                ...
                <div className="space-y-4">
                  {group.questions.map((question) => (
                    <div key={question.QuestionId} className="space-y-2">
                      <Label htmlFor={`q_${question.QuestionId}`}>{question.QuestName}</Label>

                      {/* A = Yes/No (Radio-style Select) */}
                      {question.FgAnsMode === 'A' && (
                        <Select
                          value={questionAnswers[question.QuestionId] || ''}
                          onValueChange={(v) => handleQuestionAnswerChange(question.QuestionId, v)}
                        >
                      ...
                      {question.FgAnsMode === 'N' && (
                        <Textarea
                          ...
                          value={questionAnswers[question.QuestionId] || ''}
                          onChange={(e) => handleQuestionAnswerChange(question.QuestionId, e.target.value)}
                      ...
                      {question.FgAnsMode === 'O' && (
                        <div className="relative">
                          ...
                          <Input
                            id={`q_${question.QuestionId}`}
                            type="text"
                            value={questionAnswers[question.QuestionId] ? Number(questionAnswers[question.QuestionId]).toLocaleString('id-ID') : ''}
                            onChange={(e) => handleNumericQuestionChange(question.QuestionId, e.target.value)}
                            placeholder="0"
                            className="pl-9"
                          />
                        </div>
            ...
            <div className="space-y-4 pt-4 border-t">
                 <div className="flex items-start gap-4 p-4 bg-primary/5 rounded-xl border border-primary/20">
                    ...
                    <div className="flex flex-col">
                        <Label htmlFor="is_declared_true" className="font-bold text-slate-800 text-base cursor-pointer">
                            Pernyataan Kebenaran Data*
                        </Label>
                        <p className="text-slate-600 text-sm mt-1 leading-relaxed">
                            Saya menyatakan bahwa semua informasi, data, dan dokumen yang saya berikan dalam formulir lamaran ini adalah benar, akurat, dan dapat dipertanggungjawabkan. Saya menyadari bahwa setiap pemalsuan, ketidakakuratan, atau penyembunyian fakta dapat mengakibatkan penolakan lamaran saya atau pemutusan hubungan kerja di masa mendatang.
                        </p>
                    </div>
                </div>
            </div>
              ...
              <div className="pt-4 flex justify-center">
                {captchaSiteKey ? (
                  <Turnstile
                    ...
                    onLoad={(_, boundTurnstile) => setTurnstileInstance(boundTurnstile)}
